    ...
    try:
        for model_name, model_config in MODELS.items():
            ...
            with mlflow.start_run(
                run_name=f"load_{model_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            ):
...
async def health_check():
    ...
    return {"status": "healthy", "models": models_status, "total_models": len(MODELS)}
...
async def predict(model_name: str, file: UploadFile = File(...)):
    ...
    with mlflow.start_run(
        run_name=f"predict_{model_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ):
        try:
            mlflow.log_param("model_name", model_name)
            ...
            logger.info(
                f"Prediction logged to MLFlow - Top Class: {results[0]['class_id']}, Confidence: {results[0]['confidence']:.4f}"
            )
